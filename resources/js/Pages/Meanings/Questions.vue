        ...
        <div id="wordsheetforprint1q" class="hidden p-4 bg-white rounded w-a4w h-a4h print:block">
            <div class="">
                <div class="flex flex-row text-xs">
                    ...
                    <p class="w-1/6 text-2xl italic text-right underline underline-offset-2" v-if="section == 1"><span
                            class="text-4xl">/</span>{{ testQuestions }}</p>
                </div>
                <div class="flex flex-col">
                                ...
                                <table class="min-w-full text-xs font-light text-left">
                                    ...
                                    <tbody>
                                        <tr v-for="test in splitTest" :key="test.index_no"
                                            ...
                                            <td class="px-6 py-1 text-right font-condensed whitespace-nowrap">{{
                                                test.index_no }}</td>
                                            <td class="px-6 py-2 whitespace-nowrap"
                                                :class="{ 'text-sm': testTypeIsRead, 'text-base tracking-widest': !testTypeIsRead }">{{ convertEnglish(test.word.english, testTypeIsRead, testHintLevel) }}
                                            </td>
                            ...
                            <div class="overflow-hidden">
                                <table class="min-w-full text-xs font-light text-left">
                                    <thead
                                        ...
                                        <tr>
                                            <!-- <th scope="col" class="w-1/12 px-6 py-4 pb-2">No.</th> -->
                                            <th v-if="!testTypeIsRead" scope="col" class="w-3/12 px-6 py-4 pb-2">単語</th>
                                            <th scope="col" class="w-8/12 px-6 py-4 pb-2">意味</th>
                                            <th v-if="testTypeIsRead" scope="col" class="w-3/12 px-6 py-4 pb-2">単語</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="test in splitTest" :key="test.index_no"
                                            ...
                                            <td v-if="!testTypeIsRead" class="px-6 py-2 text-sm tracking-widest whitespace-nowrap">{{ test.word.english }}</td>
                                            <td class="px-6 py-1 whitespace-nowrap"><span
                                                    class="inline-block w-6 p-1 mr-2 font-bold text-center border rounded text-xs/12">{{
                                                        test.word.part_of_speech.charAt(0) }}</span>{{ test.japanese }}
                                            </td>
                                            <td v-if="testTypeIsRead" class="px-6 py-2 text-sm whitespace-nowrap">{{ test.word.english }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
